...
- Name: smoke/login.spec.ts >> Sauce Lab Login Feature >> Invailed login
- Location: tests/smoke/login.spec.ts:27:9
...
```ts
  ...
  11 |         super(page);
  12 |         this.username = this.page.getByRole('textbox', { name: 'Username' });
  13 |         this.password = this.page.getByRole('textbox', { name: 'Password' });
  14 |         this.loginBtn = this.page.getByRole('button');
  15 |         this.errorMsg = this.page.locator('[data-test="error"]');
  16 |     }
  17 | 
  18 |     async open() {
  19 |         await this.goto("/");
  20 |     }
  21 | 
  22 |     async loginUser(user: string, pass: string) {
  23 |         await this.username.fill(user);
  24 |         await this.password.fill(pass);
  25 |         await this.loginBtn.click();
  26 |     }
  27 | 
  28 |     async verifyLogin() {
  29 |         await expect(this.page).toHaveURL(/inventory/);
  30 |     }
  31 | 
  32 |     async getError() {
> 33 |         const actualErrorMsg = await this.errorMsg.textContent();
     |                                                    ^ TimeoutError: locator.textContent: Timeout 10000ms exceeded.
  34 |         return actualErrorMsg;
  35 |     }
  36 | 
  37 | }
```